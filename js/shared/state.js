...
import { loadPool, loadRoundN, sampleRoundNames } from "./pool.js";
import { emptyRun, markAsked, pauseClock, PLAYABLE_MODES, poolSize, pointsForCorrect, runHasProgress, streakBonus } from "./run.js";
...
export function createState() {
  const selectedNames = loadPool();
  return {
    mode: MODES.HOME,
    selectedNames,
    roundN: loadRoundN(selectedNames.size),
    focusName: null,
...
export function ensureRoundPool(state, mode) {
  const run = state.runs[mode];
  if (!run) return;
  if (run.poolNames && run.poolNames.size) return;
  if (runHasProgress(run)) return;
  const sample = sampleRoundNames(state.selectedNames, state.roundN);
  if (sample) run.poolNames = sample;
}
